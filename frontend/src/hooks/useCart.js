...
const useCart = (userId) => {
  ...
  const fetchTotal = async () => {
    try {
      const response = await axios.get(`https://e-commerce-data-one.vercel.app/api/${userId}/price`);
  ...
  const minsQuantityCart = async (productId, quantity) => {
    ...
    try {
      const response = await axios.put(
        `https://e-commerce-data-one.vercel.app/api/${userId}/minscart`,
        { productId, quantity },
        {
          headers: {
  ...
  const addQuantityCart = async (productId, quantity) => {
    ...
    try {
      const response = await axios.put(
        `https://e-commerce-data-one.vercel.app/api/${userId}/addcart`,
        { productId, quantity },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
